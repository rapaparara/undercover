   ...
   persist(
      (set, get) => ({
         ...
         setTotalPlayerCount: (count) => {
            ...
            set((state) => {
               ...
               return {
                  ...
                  settings: {
                     ...state.settings,
                     numUndercover: Math.min(
                        state.settings.numUndercover,
                        maxUndercover,
                     ),
                  },
               }
         ...
         startGame: () => {
            ...
            const { assignedRoles, wordPairId, civilianWord, undercoverWord } =
               assignRoles(
                  playerIds,
                  state.settings,
                  wordPairs,
         ...
         proceedToVoting: () => {
            const state = get()
            const firstActiveIndex = state.players.findIndex(
               (p) => !p.isEliminated,
            )
            set({
               phase: 'VOTING',
               votes: {},
               currentActionIndex:
                  state.settings.votingMode === 'private'
                     ? firstActiveIndex >= 0
                        ? firstActiveIndex
                        : 0
                     : 0,
         ...
         submitVote: (voterId, targetId) => {
            ...
            if (state.settings.votingMode === 'private') {
               ...
               const currentVoterIndex = activePlayers.findIndex(
                  (p) => p.id === voterId,
               )

               if (
                  currentVoterIndex >= 0 &&
                  currentVoterIndex + 1 < activePlayers.length
               ) {
                  const nextVoter = activePlayers[currentVoterIndex + 1]
                  const nextGlobalIndex = state.players.findIndex(
                     (p) => p.id === nextVoter.id,
                  )
                  set({
                     votes: updatedVotes,
         ...
         tallyVotesAndEliminate: (manualTargetId = null) => {
            ...
            if (manualTargetId) {
               targetPlayer =
                  state.players.find((p) => p.id === manualTargetId) || null
            } else {
               // Count votes
               const voteCounts = {}
               ...
               if (topCandidates.length === 1 && maxVotes > 0) {
                  targetPlayer =
                     state.players.find((p) => p.id === topCandidates[0]) ||
                     null
               } else {
         ...
         submitMrWhiteGuess: (guess) => {
            ...
            const cleanCivilianWord = (state.civilianWord || '')
               .trim()
               .toLowerCase()
         ...
         nextRound: () => {
            ...
            if (
               state.settings.rotateStartingSpeaker &&
               activePlayers.length > 0
            ) {
               // Move speaker to next active player in join order circle
               let candidate =
                  (state.startingSpeakerIndex + 1) % state.players.length
               while (
                  state.players[candidate] &&
                  state.players[candidate].isEliminated
               ) {
         ...
         finishGame: (winner) => {
            ...
            for (const player of state.players) {
               ...
               let isWinner = false
               if (winner === 'civilian' && player.role === 'civilian')
                  isWinner = true
               if (winner === 'undercover' && player.role === 'undercover')
                  isWinner = true
               if (winner === 'mrwhite' && player.role === 'mrwhite')
                  isWinner = true
         ...
         resetAll: () => {
            ...
            if (typeof window !== 'undefined' && window.localStorage) {
               try {
                  window.localStorage.removeItem('undercover-game-storage')
               } catch (e) {
